...
4. ✅ PR 4 (Integration & lowering): wire `FunctionABI` into `backend.rs` and call lowering, add prologue/epilogue emission. Medium to Large.
   - Completed: Full integration of FunctionABI into call lowering:
     - Modified `compile_function` to compute and use FunctionABI for internal function signatures.
     - Added `function_abis` HashMap to backend state and TranslationContext.
     - FunctionABI.compute() called for all internal functions during compilation.
     - Function signatures now use `param_valtypes()` and `result_valtypes()` from FunctionABI.
     - **NEW**: Created `call_lowering.rs` module with ABI-aware call implementation:
       - `emit_abi_call()`: Dispatches to flattened or frame-based lowering based on `abi.uses_frame`.
       - `emit_flattened_call()`: Preserves existing behavior for small types (≤16 bytes).
       - `emit_frame_based_call()`: Frame allocation infrastructure (basic implementation):
         * Allocates frame with alignment (`emit_alloc_frame`).
         * Passes i32 pointers for Frame parameters.
         * Emits call with correct argument ordering.
         * Frees frame (`emit_free_frame`).
       - `handle_call_result()`: Routes result handling based on ResultABI.
     - Modified `control_flow.rs` Call statement handling to check `function_abis` and delegate to `emit_abi_call()`.
     - All existing tests pass (265 pass, 0 fail) - flattened path preserves compatibility.
     - **LIMITATION**: Frame-based infrastructure is wired but requires additional expression handling for struct/array parameters.
       Typical GLSL ES 3.0 shaders use small types that fit within 16-byte threshold, so frame path rarely needed.
   
5. 🚧 PR 5 (Polish & diagnostics): add final tests, edge-cases, complete frame parameter handling, documentation updates (`docs/10.1-calls.md`), and update WAT snapshots. Small-Medium.

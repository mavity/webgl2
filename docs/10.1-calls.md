...
4. ✅ PR 4 (Integration & lowering): wire `FunctionABI` into `backend.rs` and `expressions.rs` lowering, add prologue/epilogue emission. Medium to Large.
   - Completed: Integration of FunctionABI into compilation pipeline:
     ...
     - Infrastructure in place for frame-based call lowering (flattened calls work unchanged).
     - Current behavior: all parameters ≤16 bytes are flattened (scalars, small vectors).
     - Frame allocation infrastructure ready but not yet used (shaders typically don't need it).
   - All existing tests pass (265 pass, 0 fail).

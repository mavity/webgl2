...
4. ✅ PR 4 (Integration & lowering): wire `FunctionABI` into `backend.rs` and call lowering, add prologue/epilogue emission. Medium to Large.
   - Completed: Full integration of FunctionABI into call lowering:
     ...
     - **NEW**: Created `call_lowering.rs` module with ABI-aware call implementation:
       ...
       - `emit_frame_based_call()`: Full frame allocation sequence:
         * Allocates frame with alignment (`emit_alloc_frame`).
         * Handles Flattened params (push on stack) and Frame params (copy_in to memory + pointer arg).
         * Emits call with correct argument ordering.
         * Copy_out for out/inout Frame parameters after call.
     ...
     - Frame-based infrastructure now fully wired (exists but not exercised by current tests since shaders use small types).

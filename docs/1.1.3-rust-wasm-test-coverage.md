...
Mandatory minimal runner script

When using this repository's PoC pipeline in CI you MUST run a strict, deterministic runner that performs the following steps (no discovery, no heuristics, fail-fast on any error). This guarantees the symbolication input matches the executed binary and leaves artifacts for collection under `coverage/<crate>/`:

1. Produce Node bundle: run `wasm-bindgen --target nodejs --out-dir coverage/<crate> <wasm>` and fail on any error.
2. Capture V8 coverage: run `node scripts/node_coverage_runner.js coverage/<crate>/<crate>_bg.js` and write the raw V8 JSON to `coverage/<crate>/v8-coverage.json`.
3. Extract offsets: run `node scripts/v8cov_extract_offsets.js coverage/<crate>/v8-coverage.json > coverage/<crate>/offsets.json` and fail if empty.
4. Symbolicate: run `tools/wasm_cov_mapper --wasm <wasm> --offsets coverage/<crate>/offsets.json --out coverage/<crate>/mapped-coverage.json` and fail on error (LCOV format).
5. Finish: print the exact paths of generated files and exit with a non-zero code if any step failed, leaving artifacts in `coverage/<crate>/` for CI collection.

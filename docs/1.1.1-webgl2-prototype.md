...
- wasm_create_context_with_flags(flags: u32) -> u32 (handle, 0 on failure)
...
1. Core plumbing (Rust) — Minimal, required first (TODO #2)
   ...
   - Implement `wasm_create_context_with_flags(flags)` and `wasm_destroy_context()`.
...
3. JS class & factory (TODO #4)
   - Add `webGL2()` factory in `index.js` that instantiates wasm (or uses existing adapter) and calls `wasm_create_context_with_flags(flags)`.

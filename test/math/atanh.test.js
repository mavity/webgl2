...
test('Math Builtin: atanh', async (t) => {
    ...
    const testValues = [0.0, 0.5, -0.5, 0.9, -0.9];
    ...
    for (const val of testValues) {
        ...
        const expected = Math.atanh(val);

        assert.equal(canonicalize(result), canonicalize(expected), `atanh(${val}) failed`);
    }
});

...
test('Math Builtin: exp', async (t) => {
    ...
    const testValues = [0.0, 1.0, -1.0, 2.0, -0.5];
    ...
    for (const val of testValues) {
        ...
        assert.equal(canonicalize(result), canonicalize(expected), `exp(${val}) failed`);
    }
});

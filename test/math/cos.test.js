...
test('Math Builtin: cos', async (t) => {
    ...
    for (const val of testValues) {
        ...
        const expected = Math.cos(val);

        assert.equal(canonicalize(result), canonicalize(expected), `cos(${val}) failed`);
    }
});

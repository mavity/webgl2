...
test('Math Builtin: cosh', async (t) => {
    ...
    const testValues = [0.0, 1.0, -1.0, 0.5, -0.5];
    ...
    for (const val of testValues) {
        ...
        const expected = Math.cosh(val);

        assert.equal(canonicalize(result), canonicalize(expected), `cosh(${val}) failed`);
    }
});

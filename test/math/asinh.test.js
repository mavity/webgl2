...
test('Math Builtin: asinh', async (t) => {
    ...
    const testValues = [0.0, 1.0, -1.0, 0.5, -0.5, 10.0, -10.0];
    ...
    for (const val of testValues) {
        ...
        const expected = Math.asinh(val);

        assert.equal(canonicalize(result), canonicalize(expected), `asinh(${val}) failed`);
    }
});

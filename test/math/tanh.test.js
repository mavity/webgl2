...
test('Math Builtin: tanh', async (t) => {
    ...
    for (const val of testValues) {
        ...
        const expected = Math.tanh(val);

        assert.equal(canonicalize(result), canonicalize(expected), `tanh(${val}) failed`);
    }
});

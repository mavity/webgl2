...
// Ensure tests create/destroy contexts per subtest to avoid interference

test('IVec4 Debug Sequence', async (t) => {
  ...
  await t.test('Flat Int Triangle', async () => {
    ...
    try {
      ...
      const fs = `#version 300 es
        precision highp float; flat in int v_val; out vec4 fragColor; void main() { if (v_val == 30) fragColor = vec4(0.0, 1.0, 0.0, 1.0); else fragColor = vec4(1.0, 0.0, 0.0, 1.0); }`;
  ...
  await t.test('Flat IVec4 Triangle', async () => {
    ...
    try {
      const vs = `#version 300 es
        ...
        void main() { v_val = a_val; gl_Position = vec4(a_pos, 0.0, 1.0); }`;
      const fs = `#version 300 es
        precision highp float; flat in ivec4 v_val; out vec4 fragColor; void main() { if (v_val == ivec4(30,31,32,33)) fragColor = vec4(0.0, 1.0, 0.0, 1.0); else fragColor = vec4(1.0, 0.0, 0.0, 1.0); }`;
      const p = createProgram(gl, vs, fs);
      gl.useProgram(p);

      const values = new Int32Array([10,11,12,13, 20,21,22,23, 30,31,32,33]);

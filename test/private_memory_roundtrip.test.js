...
test('Private memory roundtrip (fragment store visible to host)', async (t) => {
  ...
  try {
    ...
    // Read back the private memory at dynamic private_ptr via the turbo global
    const mem = gl._instance.exports.memory.buffer;
    const privatePtr = gl._turboGlobals.ACTIVE_PRIVATE_PTR.value;
    const floatView = new Float32Array(mem, privatePtr, 4);

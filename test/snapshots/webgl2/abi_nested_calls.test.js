...
test('ABI: nested calls WAT or null', async () => {
  ...
  try {
    ...
    gl.shaderSource(fs, `#version 300 es
      ...
      float innerProcess(LargeData d) { return d.a.x + d.b.y + d.c.z; }
      float outerProcess(LargeData d1, LargeData d2) { return innerProcess(d1) + innerProcess(d2); }
      void main() { LargeData d; fragColor = vec4(outerProcess(d,d),0.0,0.0,1.0); }`);
    gl.compileShader(fs);
